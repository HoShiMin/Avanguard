...
/*
    RAW or Offset - offset from beginning of file
    RVA - offset in loaded module (in memory)
    VA = ImageBase + RVA - real address of something in a loaded module
    RVA -> Offset: SectionRAW + (RVA - SectionRVA)
                                            ^- Contains in a section's header
*/
...
typedef struct _IMPORT_INFO {
    ...
    PVOID FT; // FT = FirstThunk, address in IAT (real address in memory)
